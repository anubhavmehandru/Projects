																...
																<form class="form-horizontal" method="post">
																	<table class="myt">
																			<tr>
																					...
																					<td><label class="control-label">&nbsp; Call ID&nbsp;&nbsp;</label></td>
																					<td>
																						<input type="text" class="form-control" name="cid" size="15">
																					</td>
																					<td>
                                                        ...
                                                        <div class="panel-body ">
															<table class="table table-striped">
                                                              ...
                                                              <?php
																...
																{
																	$id=$_POST['cid'];
																	//$sql="select * from `call` where id in (select callid from stock where status like('%$_POST[type]%') and `callid` like('%$id%'))";
																	$sql="select * from `call` where id in (select callid from stock where status like('%$_POST[type]%')";
																	if($id!="")
																		{
																		$sql.="and `callid` in (select id from `call` where `callid`='$id' )";
																		}
																	$sql.=")";
																}
																else
