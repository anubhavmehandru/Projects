    ...
    $(document).ready(function(){
        ...
        $(".assign_txt").attr("disabled",false);
			...
			$("#adsp").click(function(){
						...
						$st+="<label class='col-md-8'>Spare Parts Required</label>";
...
<div class="col-md-12">
        <div class="panel panel-default">
                <form class="form-horizontal" method="post">
                    ...
                    <div class="pull-right">
						...
						if($_SESSION['status']=="Manager" or $_SESSION['status']=="admin")
						{
								if($row['cstatus']=="Not Assign")
								{
										echo '<a href="#assign" data-toggle="modal">
														<button class="btn btn-primary " type="button" name="sub">Assign To Engineer</button>
											</a>';
								}
						}
						if($_SESSION['status']=="Engineer" or $_SESSION['status']=="admin")
						{
								if( $row['cstatus']=="Parts Required")
								{
										echo'<a href="#estimate" data-toggle="modal">
														<button class="btn btn-primary " type="button" name="sub">Parts Required</button>
											</a>';
								}
						...
						if($_SESSION['status']=="Front Desk" or $_SESSION['status']=="admin")
						{
                ...
                <div class="panel-footer">
						<?php
								if($_SESSION['status']=="admin")
								{
									echo '<p style="color:blue;"><a href="index.php?page=view_en_3&&view='.$_GET['view'].'&&close=y">Close Call</a></p>';
								}
						?>
 ...
 <?php
	...
	{	
			...
			{
				echo "<script>window.location='index.php?page=view_en_3&&view=".$_GET['view']."';</script>";
			}
	}
	...
	if(isset($_POST['addassign']))
	{
			...
			if(mysqli_query($con,$sql))
			{
						$sql2="update `call` set `cstatus`='Parts Required' where id='$_GET[view]'";
...
if(isset($_POST['Deliver']))
	{		
			$date=date("y-m-d");
			$time=date("h:i:sa");
			$sql="insert into delivered values(null,'$_GET[view]','$date','$time','Front Desk')";
			if(mysqli_query($con,$sql))
			{
